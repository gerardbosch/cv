% Gerard Bosch long CV with moderncv package.
%  docs: https://osl.ugr.es/CTAN/macros/latex/contrib/moderncv/manual/moderncv_userguide.pdf
%  repo: https://github.com/moderncv/moderncv
%
...
\usepackage[firstyear=2010, lastyear=2026]{moderntimeline}
% \addtolength{\hintscolumnwidth}{3em} % width of the hints/timeline column
\setlength{\hintscolumnwidth}{4.1cm} % width of the hints/timeline column (10-13 words for content line, small size)
...
% ── Continuous-page PDF (two-pass) ─────────────────────────────────────────
% Pass 1: paperheight=1500mm (placeholder), textheight=1450mm. \pagetotal (the
%         actual content height) is saved to cv_pageht.tex.
% Pass 2: reads cv_pageht.tex, sets textheight = saved content height, and
%         paperheight = content + 50mm (= the fixed non-body vertical space
%         preserved from pass 1: 1500-1450=50mm). This means the footer lands
%         right below the content and TikZ overlays see the correct page size.
\makeatletter
\InputIfFileExists{cv_pageht.tex}{}{}  % defines \cvPageTotalStr from previous run
\@ifundefined{cvPageTotalStr}{%
  \usepackage[scale=0.8, paperheight=1500mm, textheight=1450mm]{geometry}%
}{%
  \newlength{\@cvpaperht}%
  \setlength{\@cvpaperht}{\cvPageTotalStr}%
  \addtolength{\@cvpaperht}{50mm}%
  \edef\@cvpaperhtstr{\the\@cvpaperht}%
  \usepackage[scale=0.8, textheight=\cvPageTotalStr, paperheight=\@cvpaperhtstr]{geometry}%
}
\newwrite\@cvhtfile
\AtBeginDocument{%
  \let\ori@clearpage\clearpage
  \renewcommand\clearpage{%
    \par
    \ifdim\pagetotal>0pt
      \immediate\openout\@cvhtfile=cv_pageht.tex
      \immediate\write\@cvhtfile{\string\def\string\cvPageTotalStr{\the\pagetotal}}%
      \immediate\closeout\@cvhtfile
    \fi
    \ori@clearpage
  }%
}
...
\usepackage{lastpage}
\newcommand{\cvbuildinfo}{%
  \href{https://github.com/gerardbosch/cv}{\textcolor{gray}{CV made with \LaTeX~/ built by \faGithub*~GitHub Actions / delivered via \faCloudflare~Cloudflare}}%
}
\cfoot{\cvbuildinfo}
%\rfoot{\addressfont\itshape\textcolor{gray}{Page \thepage\ / \pageref*{LastPage}}} % disable for continous scroll (single page)
\rhead{\cvbuildinfo\hspace{1.0cm}}
%\rhead{\addressfont\itshape\textcolor{gray}{Page \thepage\ / \pageref*{LastPage}}} % disable for continous scroll (single page)
...
%----------------------------------------------------------------------------------
% Double item with content length option
%----------------------------------------------------------------------------------
\makeatletter
\define@key{cvdoubleitem}{cvdiItemSep}{\def\cvdoubleitem@iItemSep{#1}}
\define@key{cvdoubleitem}{cvdiAddToLen}{\def\cvdoubleitem@addToLen{#1}}
\newcommand*{\cvdoubleitemMy}[5][]{%
  \setkeys{cvdoubleitem}{cvdiItemSep=0.5em, cvdiAddToLen=0pt, #1}%
  \cvitem[\cvdoubleitem@iItemSep]{\textcolor{color1}{\textbf{#2}}}{% % Hint-1
    % Content-1
    \begin{minipage}[t]{\doubleitemcolumnwidth+\cvdoubleitem@addToLen}\small#3\end{minipage}%
    \hfill% fill of \separatorcolumnwidth
    % Hint-2
    \begin{minipage}[t]{\hintscolumnwidth-\cvdoubleitem@addToLen*2}\raggedleft\hintstyle{\textcolor{color1}{\textbf{#4}}}\end{minipage}%
    \hspace*{\separatorcolumnwidth}%
    % Content-2
    \begin{minipage}[t]{\doubleitemcolumnwidth+\cvdoubleitem@addToLen}\small#5\end{minipage}}}
\makeatother
...
\newcommand{\forkme}{
  \begin{tikzpicture}[remember picture, overlay]
    \node[rotate=45, shift={(0, -2.9cm)}] at (current page.north west) {
      \begin{tikzpicture}[remember picture, overlay]
...
\firstname{Gerard}
\familyname{Bosch}
\title{Software Engineer,\\ \large{Freelancer/Contractor}} % optional, remove the line if not wanted
\address{\faGlobeAmericas~ US/EU Worldwide remote}    % optional, remove the line if not wanted
...
\extrainfo{
  ...
  \faLaptop ~\href{https://gerardbosch.xyz}{gerardbosch.xyz}
}
...
[ \emph{Read the one-page résumé version at} \href{https://resume.gerardbosch.xyz}{resume.gerardbosch.xyz} ]
...
\vspace{1ex}
\begin{adjustwidth}{1.7cm}{2.8cm}
  
  \setlength{\parskip}{8pt}
  
  % \hangpara does not work in this environment :-/ use a parbox
  \larrow{color1}
  ...
  \larrow{color1}
  \parbox[t]{0.97\linewidth}{
    Good at identifying inefficiencies, \textcolor{brown}{finding solutions} and \textcolor{brown}{improving processes}. I enjoy collaborating with diverse teams to solve challenging problems.
  }
\end{adjustwidth}
...
\vspace{-3pt}\cventry{2020}{\href{https://bit.ly/fp-short-intro}{\textnormal{Functional Programming super-short intro}}}{}{}{}{}
\vspace{-4pt}\cventry{2018}{\href{https://blockchain-presentation.gerardbosch.xyz}{\textnormal{A technical introduction to Blockchain technology}}}{}{}{}{}


% \vspace{-0.246cm}
\section{\textsc{Professional Experience}}

\tlcventry{2025}{0}{Software Engineer | Messaging + Recommendation}{Leboncoin.fr · Freelance}{Remote}{}{
  \emoji{bullseye} \emph{Support the migration and handover of Adevinta's central messaging (chat) backend platform to Leboncoin, ensuring smooth transition to full ownership and autonomy. The system supports 20M+ monthly active users, tens of millions of conversations per month, and millions of daily messages through an Event-Driven Architecture (Kafka, Kafka Streams).} \\[1ex]
  \emoji{bullseye} \emph{Created an architectural framework (Spring based) to be an umbrella of 30+ microservices and centralize service management, versioning and evolution, providing cross-cutting components for the Event-Driven system, and make maintenance and upgrades much easier. It strongly contributed to harmonize code.} \\[1ex]
  \vspace{-3ex}
  \begin{itemize}\setlength\itemsep{0pt}
    \item Actively engage in the roadmap for 2026 through actionable initiatives.
    \item Develop new features and modernize the codebase and microservices architecture to meet evolving business and system requirements, improving scalability, reliability, and maintainability.
    \item Operate, troubleshoot, and optimize the system to ensure high availability, performance, and resilience in a large-scale distributed environment. Bridge original and new teams, driving knowledge transfer, collaboration, and alignment for successful migration.
  \end{itemize}
  % \vskip 0pt ends the paragraph without a \par token (\cventry is \newcommand*).
  % \hangpara must stay ungrouped — a wrapping {…} would restore \hangindent to 0
  % before \par fires and kill the hanging indent.
  \noindent After the first year I switched to the Recommendation team to work on the Golang Backend for the Homepage \& Recommendation team which serves high-volume traffic and provides user personalized recommendations; interfacing with Frontend, ML and vector DB for ANN search and semantic search.
  \vskip 1em
  \hangpara{1.5em}{1}\textbf{Keywords}: \textit{Kotlin, Spring Boot, Kafka, Kafka Streams, Kubernetes, ScyllaDB, Postgres, Clickhouse, AWS, Go, Milvus/Zilliz\ldots}\\
}

\tlcventry{2024}{2025}{Backend Software Engineer | Quarkus, Reactive}{Tymit}{Remote}{}{
  \emoji{bullseye} \emph{Authored an RFC with actionable proposals to align the current architecture with Clean Architecture™ \& SOLID principles, with a special focus on the Interface Segregation (ISP) and Single Responsibility Principle (SRP), plus other enhancements.} \\[-3ex]
  \begin{itemize}\setlength\itemsep{0pt}
    \item Engaged with the integrations team to support the migration of hundreds of thousands of customers into our system, while also developing new public-facing APIs to enable a new business model requiring first-party and third-party B2B backend integrations.
    \item Improved testing reliability by introducing Property-Based Testing and showcasing its principles to the backend guild, ensuring more robust testing including edge-cases, generating thousands of test values.
    \item Spearheaded API contract segregation into dedicated modules, enabling the publication of versioned API artifacts for seamless and efficient cross-service integration using Gradle, contributing to DX.
    \item Shared coding best practices and actively contributed to code reviews and BE discussions.
  \end{itemize}
  \hangpara{1.5em}{1}\textbf{Keywords}: \textit{Quarkus, Microservices, Reactive Programming, AWS (ECS, DynamoDB, SQS, SNS, RDS), Liquibase,…} \\
}

\tlcventry{2022}{2023}{(Break) / Software Engineer}{Self employed}{Barcelona}{}{
  \emoji{bullseye} \emph{I set up my own company to provide software engineering services as a contractor.} \\[-3ex]
  \begin{itemize}\setlength\itemsep{-0.5pt}
    \item Algorithmic trading, research, design \& implementation of a risk management system for loss control, as well as key metrics collection/reporting using DDD \& Hexagonal arch.
    \item High level design of a multi-level affiliation system with referral fee distribution.
    \item Took a break from industry to learn about finances, markets and investments.
  \end{itemize}
  \vspace{-3pt}
  % \hangpara{1.5em}{1}\textbf{Keywords}: \textit{MQL, C++.}
}

\tlcventry{2021}{2022}{Software Engineer}{N26 Bank}{Barcelona}{}{
  \emoji{bullseye} \emph{I contributed to streamline the processing of banking fees with the goal of having a more centralized and well-defined domain for charging fees.} \\[1ex]
  \null
  ...
  \begin{itemize}\setlength\itemsep{0pt}
    ...
    \item Implement new business use cases as per new fee creation or regulatory compliance.
...
\tlcventry{2016}{2021}{Senior Software Engineer}{GFT IT Consulting}{Lleida/Barcelona}{}{
  \vspace{-3pt}
  \emoji{bullseye} \emph{Successfully delivered a large and complex regulatory banking project to provide external, permission-based access to banking APIs, ensuring on-time completion. Then, I was called to contribute building a new bank architecture for high-concurrency, based on acquired know-how.} \\[3pt]
  PROJECT HIGHLIGHTS (most relevant projects only):
  \begin{itemize}\setlength\itemsep{4pt}
     ...
     \item \textit{Banc Sabadell}: PSD2 \& APIfication (2018--2020) \\[2pt]
           ...
           • Design and implement the APIs for the European regulatory project
             \href{https://en.wikipedia.org/wiki/Payment_Services_Directive}{\textit{Payments Service Directive 2}}/\textsc{psd2}
             (\textit{\href{https://en.wikipedia.org/wiki/Strong_customer_authentication}{Strong Customer Authentication}}/\textsc{sca}, Open Banking, Fintech integration,\ldots);
             leading the first approach to API exposition for the \textit{Banc Sabadell}. I worked both on the API definition side and the Architecture side, defining and implementing architectural components for the Microservices Architecture, where I was established as the technical lead for the development team until post-production. \\[2pt]
           • Defined and set up an \emph{API-First} approach with OpenAPI and API contracts generation (interfaces + \textsc{dto}) with
...
\cvdoubleitemMy[cvdiAddToLen=37pt]
  {Programming\\ Languages}{Kotlin, Scala, Java, Bash, Haskell, Python, Perl, SQL.}
  {Interest\\ Areas}{
    Functional Programming,
    \hrefcol[gray]{https://www.unison-lang.org/docs/the-big-idea/}{\dashuline{Unison lang}},
    Effect Systems,
    \hrefcol[gray]{https://zio.dev/}{\dashuline{ZIO}},
    \hrefcol[gray]{https://getkyo.io}{\dashuline{Kyo}},\ldots \\
    Big\slash Fast Data \& Streaming, DLTs\slash Blockchain \& Web3, Event-Driven Architectures, Artificial Intelligence, WebAssembly.
  }

\cvdoubleitemMy[cvdiAddToLen=37pt]
  {Development Methodologies}{
    FP patterns, GoF design patterns, SOLID, Clean Code, Hexagonal\slash Clean Architecture, 12-Factor,
    \hrefcol[gray]{https://archive.ph/qVVD5}{\dashuline{ADTs}},
    Programming with types,
    \hrefcol[gray]{https://fsharpforfunandprofit.com/rop/}{\dashuline{Railway Oriented Prog.}},
    Agile development, SCRUM, Domain Driven Design, TDD.
  }
  {Others}{
    Cloud-Native, AWS, Git, Maven\slash Gradle\slash sbt, JGitVer, Docker, OpenShift, Devcontainers, Testcontainers, DevOps, GitHub, GitLab, Cloudflare, \LaTeX, Linux, Nix, Microservices, Cloud Native, OpenAPI, AsyncAPI, API first, oAuth.
  }
...
\cvline{}{\parbox{4cm}{\href{https://repositori.udl.cat/bitstream/handle/10459.1/45832/Bosch.pdf}{\emoji{notebook-with-decorative-cover} Dissertation »}}\footnotesize{(Catalan + English full configuration guide)}}
\cvline{}{\parbox{4cm}{\href{https://www.slideshare.net/rockingerard/sauron-system-implementation-and-deployment-for-dns-management}{\emoji{desktop-computer} Presentation »}}\footnotesize{(English)}}
...
\cvline{}{\textsl{\textbf{Qualified} with honors.}}
\cvline{}{\href{https://repositori.udl.cat/bitstream/handle/10459.1/45751/Bosch.pdf}{\emoji{notebook-with-decorative-cover} Dissertation »}}
\cvline{}{\href{https://www.slideshare.net/rockingerard/implementaci-dels-codis-de-golay-en-sage}{\emoji{desktop-computer} Presentation »}}
